...
async def handle_click(
    page: Any, input: ClickInput, memory: SessionMemory
) -> ToolResult:
    del memory
    try:
...
async def handle_press(
    page: Any, input: PressInput, memory: SessionMemory
) -> ToolResult:
    del memory
    try:
        locator = resolve_locator(page, input.locator)
...
async def handle_hover(
    page: Any, input: HoverInput, memory: SessionMemory
) -> ToolResult:
    del memory
    try:
        locator = resolve_locator(page, input.locator)
...
async def handle_check(
    page: Any, input: CheckInput, memory: SessionMemory
) -> ToolResult:
    del memory
    try:
        locator = resolve_locator(page, input.locator)
...
async def handle_scroll(
    page: Any, input: ScrollInput, memory: SessionMemory
) -> ToolResult:
    del memory
    try:
    ...
    try:
        ...
        if not input.exact and input.expected not in actual:
            return ToolResult(
                ok=False,
                error=(
                    f"Expected text containing '{input.expected}' but found '{actual}'"
                ),
                retryable=False,
            )
